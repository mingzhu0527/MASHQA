...
#         k = k_enc_outputs.shape[2].value
        ...
        with tf.variable_scope('attention_layer'):
            ...
            proj_q = tf.matmul(k_enc_outputs, q_enc_outputs, transpose_b=True) # transpose_b means transpose second var
            # Normalize
...
def preprocess_val():
    ...
    sp_model.Load(FLAGS.spiece_model_file)
    spm_basename = _get_spm_basename()
    eval_examples = read_squad_examples(FLAGS.predict_file, is_training=True)
    ...
    if tf.gfile.Exists(eval_rec_file) and tf.gfile.Exists(
        ...
        eval_writer = FeatureWriter(filename=eval_rec_file, is_training=True)
        eval_features = []
...
#     hsz = tf.shape(output)[2]
    ...
    # nsent*bsz, hsz
    sent_rep_vectors = AttentionPoolingLayer(initializer)(sent_output, sent_output)
    # 16, b, 1024
