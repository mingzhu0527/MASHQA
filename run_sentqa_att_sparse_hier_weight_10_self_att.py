...
class AttentionNoPoolingParameterizedSparseLayer(object):
...
#             proj_q = tf.nn.softmax(tf.multiply(proj_q, 1/math.sqrt(k)))
            # b l l
            proj_q0_norm = proj_q0/tf.math.sqrt(tf.norm(proj_q0))
#             proj_q1 = tf.nn.softmax(tf.multiply(proj_q0, 1.0/(4.0*k)))
            # b l l
            proj_q1 = entmax15(proj_q0_norm)
            
            # b l 2h
            proj_q = tf.matmul(proj_q1, q_enc_outputs)
                    
       
        return proj_q, proj_q0, proj_q0_norm, proj_q1
...
#     hsz = tf.shape(output)[2]
    ...
    sent_rep_cls_att, proj_q0, proj_q0_norm, proj_q1 = AttentionNoPoolingParameterizedSparseLayer()(sent_rep_cls, sent_rep_cls)
#     sent_rep_cls_att = AttLayer(initializer, 512, 2*xlnet_config.d_model)(sent_rep_cls)
    # b, 13, 4h
    x_final = tf.concat([sent_rep_cls, sent_rep_cls_att], -1)
...
#     return_dict["proj_q0_norm"] = proj_q0_norm
...
#                     "proj_q0_norm": outputs["proj_q0_norm"],
